...
type Prospect = {
  id: string; email: string; nom: string | null; departement: string | null;
...
export function ProspectionClientsClient() {
  ...
  async function importer(file: File) {
    ...
    try {
      ...
      const idx = {
        ...
        nom: entetes.findIndex((h) => h.includes("nom") || h.includes("societe") || h.includes("société") || h.includes("asso") || h.includes("contact")),
  ...
  function demanderRelance(p: Prospect) {
    ...
    setDialogue({
      titre: `Relancer ${p.nom ?? p.email} ?`,
      lignes: [
        `Un email de relance (version courte) partira à ${p.email}.`,
  ...
  async function basculerStop(p: Prospect) {
    ...
    await fetch("/api/admin/prospection-clients", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <main className="max-w-6xl mx-auto px-7 py-12">
        ...
        <div className="flex flex-wrap items-end gap-5">
          <div>
            ...
            <p className="font-mono text-[10.5px] text-blanc-faint mt-1.5">
              Colonnes détectées automatiquement : email (obligatoire), nom/société/association, département.
            </p>
          </div>
          <div className="ml-auto flex items-end gap-3">
            <a href="/api/admin/prospection-clients?preview=1" target="_blank" className="btn-ghost">
      ...
      <div className="flex items-center gap-4 mb-4">
        <input className="input flex-1" placeholder="Rechercher (email, nom, département…)"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-ligne font-mono text-[10.5px] uppercase tracking-wider text-blanc-faint">
              ...
              <th className="text-left px-5 py-3">Nom / Association</th>
          ...
          <tbody>
            {visibles.map((p) => (
              <tr key={p.id} className="border-b border-ligne/50 last:border-0">
                ...
                <td className="px-5 py-2.5 text-blanc-dim">{p.nom ?? "—"}</td>
